...
DEFAULT_Z_MIN_VALID = 0.30
DEFAULT_Z_MAX_VALID = 5.00
    ...
    def callback_distance(self, msg_point2, msg_bb, msg_mask):
        ...
        for bb in msg_bb.boxes:
            ...
            if valid_depths.size < self.min_valid_mask_pixels:
                rospy.logwarn(
                    "[FastSAM DEPTH] Medida descartada: pocos píxeles válidos "
                    "en máscara final (%d/%d). NO se usa bbox completa.",
                    valid_depths.size,
                    self.min_valid_mask_pixels
                )
                continue
            ...
            if z_spread > self.max_z_spread:
                rospy.logwarn(
                    "[FastSAM DEPTH] Z descartada por dispersión | "
