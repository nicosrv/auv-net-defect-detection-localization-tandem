...
class VerticalInspector(object):
    ...
    def bbox_cb(self, msg):
        ...
        Importante:
        - Esto NO cambia el objetivo 3D.
        - Solo se usa durante APPROACH_HOLE para centrar visualmente el agujero.
        """
    ...
    def hole_cb(self, msg):
        ...
        # Objetivo congelado. No aceptamos nuevos /hole como objetivo.
        if self.state in ["APPROACH_HOLE", "READY_TO_CROSS", "FINISHED"]:
            return
    ...
    def main_control_loop(self):
        ...
        # --- State 4: APPROACH_HOLE ---
            ...
            if self.hole_pose is None:
                self.publish_cmd(0.0, 0.0, 0.0, 0.0, "inspector_waiting_hole")
                return
            ...
            if bbox_is_recent:
                ...
                # SIGNO DE YAW CORREGIDO:
                # Esta es la corrección que hizo que el error horizontal
                # empezara a reducirse en vez de crecer.
                wz = self.clip(
                    self.kp_visual_yaw * err_img_x,
                    -self.wz_max,
                    self.wz_max
                )
                ...
                # Solo avanzamos si el agujero está visualmente centrado.
                ...
                self.publish_cmd(vx, 0.0, vz, wz, "inspector_visual_approach")

                rospy.loginfo_throttle(
                    1.0,
                    "Approach VISUAL | ImgErrX: %.2f ImgErrY: %.2f | Centered(%s,%s) | ErrDist: %.2fm | Cmd(vx:%.2f vz:%.2f wz:%.2f)",
                    err_img_x,
                    err_img_y,
                    str(centered_x),
                    str(centered_y),
